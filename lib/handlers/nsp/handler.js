...
var CLIENT_SCRIPT = '<script src="/-rc-/nsp-client.js"></script>';
...
//创建 mvc 模板扩展对象
Handler.prototype._createTemplateExtend = function (presenter) {
    var self = this;
    return {
        "self": presenter
    };
...
Handler.prototype.handleByPresenter = function (context, Presenter, templatePath) {
    ...
    var _args = JSON.parse(context.data(HIDDEN_ARGS) || '[]');
    if (!presenter[_method]) {
        context.error('没有找到名为 "' + _method + '" 的方法');
    }
    //页面呈现方法
    presenter.render = function () {
        //在 nsp 的模板中 this 指向 presenter
        var content = pagine.execFile(templatePath, presenter, {
            "extend": self._createTemplateExtend(presenter)
        });
        self.content(context, presenter, content);
    };
    //准备就绪方法，此方法将触发 method 的调用
    //如果定义了 init 方法，需要手动调用 ready
    presenter.ready = function () {
        //防止重复调用开始
        if (presenter.__ready_called) {
            return;
        }
        presenter.__ready_called = true;
        //防止重复调用结束
        presenter[_method].apply(presenter, _args);
    };
    //--
    context.filterInvoker.invoke('onNspHandle', context, function () {
        //调用 init 方法
        if (presenter.init) {
            presenter.init(presenter.ready);
        } else {
            presenter.ready();
        }
    });
...
Handler.prototype.content = function (context, presenter, content) {
    var self = this;
    context.canCompress = self.configs.nsp.compress;
