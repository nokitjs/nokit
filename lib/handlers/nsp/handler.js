...
const base64 = require("nb64");
const Cahce = require("smache");

const SERVER_ID_NAME = "nsp-id";
const HIDDEN_METHOD = "__method";
const HIDDEN_STATE = "__state";
const HIDDEN_ARGS = "__args";
const ALLOWED_METHODS = ["GET", "POST"];
const CLIENT_SCRIPT = '<script src="/-rc-/javascripts/nsp-client.js"></script>';
const CHCHE_MAX_NUM = 100;

const DefaultPresenter = function () { };
const PresenterCache = new Cahce({
  max: CHCHE_MAX_NUM
});
...
Handler.prototype.handle = function (context) {
  ...
  context.request.physicalPathExists(function (exists) {
    if (exists) {
      ...
      //检查扩展名
      if (path.extname(templatePath) != '.nsp') {
      ...
      //--
      var Presenter = PresenterCache.get(presenterPath);
      if (Presenter) {
        self.handleByPresenter(context, Presenter, templatePath);
        return;
      }
      fs.exists(presenterPath, function (exists) {
        Presenter = exists ? require(presenterPath) : DefaultPresenter;
        self.generator.wrap(Presenter.prototype);
        PresenterCache.set(presenterPath, Presenter);
        self.handleByPresenter(context, Presenter, templatePath);
      });
    } else {
      self.next(context);
