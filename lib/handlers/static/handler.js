...
Handler.prototype.handleDefaultFile = function(context, defaultFile) {
    ...
    if (url.indexOf('?') > -1) {
        var urlParts = url.split('?')
        context.request._setUrl(urlParts[0] + defaultFile + "?" + urlParts[1]);
    } else {
        context.request._setUrl(context.request.url + defaultFile);
    }
    /*
...
Handler.prototype.writeFolder = function(context) {
    ...
    fs.readdir(context.request.physicalPath, function(err, files) {
        ...
        context.responseTemplate("dir", model, self.configs.mimeType['.html']);
    });
};
...
Handler.prototype.writeFile = function(context) {
    ...
    if (self.configs.cache.client.modifiedSince) {
        fs.stat(filename, function(err, stat) {
            ...
            if (modifiedSince == null || modifiedSince < stat.mtime) {
                var fileReadStream = fs.createReadStream(filename);
                context.responseContent(fileReadStream, null, null, {
        ...
        var fileReadStream = fs.createReadStream(filename);
        context.responseContent(fileReadStream);
    }
};