...
Handler.prototype.handleDefaultFile = function(context, defaultFile) {
    var self = this;
    context.request.setPhysicalPath(defaultFile);
    var url = context.request.url;
    if (url.indexOf('?') > -1) {
        var urlParts = url.split('?')
        context.request.setUrl(urlParts[0] + defaultFile + "?" + urlParts[1]);
    } else {
        context.request.setUrl(context.request.url + defaultFile);
    }
    /*
...
Handler.prototype.writeFolder = function(context) {
    ...
    fs.readdir(context.request.physicalPath, function(err, files) {
        ...
        context.template("dir", model, self.server.mime('.html'));
    });
};
