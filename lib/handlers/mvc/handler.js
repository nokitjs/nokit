var fs = require("fs");
var path = require("path");
var GeneralController = require("./general-controller");

var ALLOWED_METHODS = ["GET", "POST"];
var DEFAULT_ACTION = "index";

var Handler = module.exports = function (server) {
    var self = this;
    self.server = server;
    self.configs = self.server.configs;
    self.configs.mvc = self.configs.mvc || {};
    self.utils = self.server.utils;
    //检查并创建视图引擎
    self.viewEngine = server.viewEngine;
    //初始化 router
    self.router = new server.Router(self.configs.mvc.routes);
    //初始化 controller / view 存放根路径
    self.configs.mvc.paths = self.configs.mvc.paths || {};
    self.paths = {};
    self.paths.controller = self.server.resolvePath(self.configs.mvc.paths.controller || './');
    self.paths.view = self.server.resolvePath(self.configs.mvc.paths.view || './');
};

//处理请求
Handler.prototype.handle = function (context) {
    var self = this;
    //console.log('请求方法：' + context.request.method);
    var route = self.router.get(context.request.withoutQueryStringURL, true);
    //如果找不到匹配路由，转交到默认处理器
    if (!route) {
        self.next(context);
        return;
    }
    // mvc 根据路由配置决定支持的 http method，默认为 get、post
    route.methods = route.methods || ALLOWED_METHODS;
    var httpMethod = context.request.method;
    if (route.methods.indexOf(httpMethod) < 0) {
        context.notAllowed();
        return;
    }
    //扩展 context
    context.route = route;
    context.routeData = route.data;
    //控制器处理
    var controller = null;
    //加载 controller 
    if (route.controller) {
        var controllerFile = path.resolve(self.paths.controller, route.controller);
        //console.log(controllerFile);
        var Controller = require(controllerFile);
        controller = new Controller(context);
    } else if (route.view) {
        //如果直接在路由中配置了 view 而没有配置 controller
        controller = new GeneralController(context);
        controller.view = route.view;
    } else {
        return context.error('路由 "' + route.pattern + '" 配置错误');
    }
    //检查 action 是否存在
    var action = context.route.action || DEFAULT_ACTION;
    /**
     * 如果 action 包含在 url 中，没有找到 action 时直接 next
     * 如果 action 为默认 index 或 “明确指定”，则提示 action 没有找到
     **/
    if (!controller[action] && route.actionInUrl) {
        return self.next(context);
    } else if (!controller[action]) {
        return context.error('没有找到名为 "' + action + '" 的 Action');
    }
    controller.context = context;
    controller.session = context.session;
    controller.server = context.server;
    controller.response = context.response;
    controller.request = context.request;
    controller.route = context.route;
    controller.locale = context.locale;
    //渲染方法
    controller.render = function (viewPath, model) {
        var viewFile = path.resolve(self.paths.view, viewPath);
        //在 mvc 的模板中 this 指向 model
        var buffer = self.viewEngine.executeFile(viewFile, model, {
            "extend": {
                "controller": controller,
                "context": context,
                "locale": context.locale,
                "route": route,
                "server": context.server,
                "session": context.session
            }
        });
        context.canCompress = self.configs.mvc.compress;
        context.content(buffer, 'text/html');
    };
    //准备就绪方法，此方法将触发 action 的调用
    //如果定义了 init 方法，需要手动调用 ready
    controller.ready = function () {
        //防止重复调用开始
        if (controller.__ready_called) {
            return;
        }
        controller.__ready_called = true;
        //防止重复调用结束
        //调用 action
        controller[action]();
    };
    //--
    context.filterInvoker.invoke('onMvcHandle', context, function () {
        //调用 init 方法
        if (controller.init) {
            controller.init(controller.ready);
        } else {
            controller.ready();
        }
    });
};
/*end*/