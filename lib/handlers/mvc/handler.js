var fs = require("fs");
var path = require("path");

var ALLOWED_METHODS = ["GET", "POST"];

var pagine = null;

var Handler = module.exports = function (server) {
    var self = this;
    self.server = server;
    self.configs = self.server.configs;
    self.configs.mvc = self.configs.mvc || {};
    self.utils = self.server.require('$./core/utils');
    //检查并创建视图引擎
    pagine = pagine || ((function () {
        var Pagine = self.server.require('$./core/pagine');
        return new Pagine();
    })());
    //初始化 router
    self.router = new server.Router(self.configs.mvc.routes);
    //初始化 controller / view 存放根路径
    self.configs.mvc.paths = self.configs.mvc.paths || {};
    self.paths = {};
    self.paths.controller = self.server.resolvePath(self.configs.mvc.paths.controller || './');
    self.paths.view = self.server.resolvePath(self.configs.mvc.paths.view || './');
};

//解析路由动态 action
Handler.prototype._parseAction = function (route) {
    var self = this;
    if (route &&
        route.action &&
        route.action.indexOf('{') > -1 &&
        route.action.indexOf('}') > -1) {
        self.utils.each(route.data, function (key, val) {
            route.action = self.utils.replace(route.action, "{" + key + "}", val);
        });
    }
    return route;
};

//匹配路由
Handler.prototype._matchRoute = function (url) {
    var self = this;
    var route = self.router.get(url);
    //如果直接成功匹配
    if (route) {
        return self._parseAction(route);
    }
    /*
    如果直接匹配不成功，
    则将 “/” 分隔的最后一个 “字串” 当作 action 进行再一次匹配
    */
    var urlParts = url.split('/');
    var lastIndex = urlParts.length - 1;
    var action = urlParts[lastIndex];
    var newUrl = urlParts.slice(0, lastIndex).join('/');
    route = self.router.get(newUrl);
    //如果查找到
    if (route) {
        route.action = action;
        return self._parseAction(route);
    } else {
        return null;
    }
};

//创建 mvc 模板扩展对象
Handler.prototype._createTemplateExtend = function (controller) {
    var self = this;
    return {
        "controller": controller
    };
};

//处理请求
Handler.prototype.handle = function (context) {
    var self = this;
    //console.log('请求方法：' + context.request.method);
    var route = self._matchRoute(context.request.withoutQueryStringURL);
    //如果找不到匹配路由，转交到默认处理器
    if (!route) {
        self.next(context);
        return;
    }
    // mvc 根据路由配置决定支持的 http method，默认为 get、post
    route.methods = route.methods || ALLOWED_METHODS;
    var httpMethod = context.request.method;
    if (route.methods.indexOf(httpMethod) < 0) {
        context.notAllowed();
        return;
    }
    //扩展 context
    context.route = route;
    context.routeData = route.data;
    //控制器处理
    var controllerFile = path.resolve(self.paths.controller, route.target);
    //console.log(controllerFile);
    var Controller = require(controllerFile);
    var controller = new Controller(context);
    //检查 action 是否存在
    var action = context.route.action || "index";
    if (!controller[action]) {
        context.error('没有找到名为 "' + action + '" 的 Action');
        return;
    }
    controller.context = context;
    //渲染方法
    controller.render = function (viewPath, model) {
        var viewFile = path.resolve(self.paths.view, viewPath);
        //在 mvc 的模板中 this 指向 model
        var buffer = pagine.execFile(viewFile, model, {
            "extend": self._createTemplateExtend(controller)
        });
        context.canCompress = self.configs.mvc.compress;
        context.content(buffer, 'text/html');
    };
    //准备就绪方法，此方法将触发 action 的调用
    //如果定义了 init 方法，需要手动调用 ready
    controller.ready = function () {
        //防止重复调用开始
        if (controller.__ready_called) {
            return;
        }
        controller.__ready_called = true;
        //防止重复调用结束
        //调用 action
        controller[action]();
    };
    //--
    context.filterInvoker.invoke('onMvcHandle', context, function () {
        //调用 init 方法
        if (controller.init) {
            controller.init(controller.ready);
        } else {
            controller.ready();
        }
    });
};
/*end*/