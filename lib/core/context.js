...
var BufferLogger = require("./buffer-logger");
...
Context.prototype._handleLogger = function () {
    var self = this;
    self.logger = new BufferLogger(self.server.logger);
};
...
Context.prototype._hookResponse = function () {
    ...
    self.onEnd = function (callback) {
        self.isEnd = true;
        self.filterInvoker.invoke('onRequestEnd', self, callback);
    };
...
Context.prototype._handleSession = function () {
    ...
    if (!self.sessionId) {
        ...
        res.cookie.add(SID_COOKIE_NAME, self.sessionId, {
            "httpOnly": true,
...
Context.prototype._handleLocale = function () {
    var self = this;
    self.locale = self.server.locale.getByContext(self) || {};
};
...
Context.prototype.setLocale = function (localeName) {
    var self = this;
    self.locale = self.server.locale.get(localeName) || {};
};
...
Context.prototype.stream = function (contentStream, mime, statusCode, headers) {
    var self = this;
    //防止重复调用开始
    if (self.__stream_called) {
        return;
    }
    self.__stream_called = true;
    //防止重复调用结束
 ...
 * 向浏览器输出文本或流
 **/
Context.prototype.send = function () {
    var self = this;
    self.content.apply(self, arguments);
};
...
Context.prototype.json = function (jsonObject) {
    ...
    self.content(jsonText, self.server.mime('.json'));
};
...
Context.prototype.jsonp = function (jsonObject) {
    ...
    var jsonpCallback = self.data(self.configs.jsonpParam);
    if (jsonpCallback) {
        self.content(jsonpCallback + "(" + jsonText + ")", self.server.mime('.js'));
    } else {
