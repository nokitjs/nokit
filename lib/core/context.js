...
Context.prototype.responseTemplate = function(template, model, mime, statusCode) {
    var self = this;
    ...
    self.responseContent(templateFunc(model), mime, statusCode);
};

Context.prototype.responseStatus = function(statusCode, options) {
    var self = this;
    options = options || {};
    var mime = self.configs.mimeType['.html'];
    var template = options.message ? tp.compile(options.message) : statusCode;
    var model = options.model || {};
    model.context = self;
    model.server = self.server;
    model.request = self.request;
    model.response = self.response;
    self.responseTemplate(template, model, mime, statusCode);
};
...
Context.prototype.responseNotFound = function(message) {
    var self = this;
...
Context.prototype.checkCache = function(type) {
    var self = this;
    if (type == CACHE_TYPE_SERVER && !utils.isNull(self.canServerCache)) {
        return self.canServerCache;
    }
    if (type == CACHE_TYPE_CLIENT && !utils.isNull(self.canClientCache)) {
        return true;
    }
...
Context.prototype.responseStream = function(contentStream, mime, statusCode) {
    var self = this;
...
Context.prototype.responseContent = function(content, mime, statusCode) {
    var self = this;
    self.response.mime = mime || self.request.mime;
    var isStream = (content instanceof Stream);
    if (isStream) {
        self.responseStream(content, mime, statusCode);
    } else {
        ...
        self.responseStream(contentStream, mime, statusCode);
    }
};
