...
var Context = function (server, req, res) {
    ...
    self.request = self.req = new Request(self, req);
    self.response = self.res = new Response(self, res);
 ...
 **/
Context.prototype.param = function (name) {
    var self = this;
    self.route = self.route || {};
    var params = self.route.data || {};
    var query = self.request.query || {};
    var form = self.request.body || self.request.form || {};
    var files = self.request.files || {};
    return params[name] || query[name] || form[name] || files[name];
};
