...
const BufferStream = require('bufstream');
 ...
 * http 请求上下文对象
 */
const Context = function (server, req, res) {
  var self = this;
  //基本成员
...
Context.prototype.stream = function (contentStream, mime, statusCode, headers) {
  ...
  self.filterInvoker.invoke('onResponse', self, function (err) {
    ...
    //处理客户端缓存开始
    var maxAge = self._checkCache() ? (self.configs.cache.maxAge || 0) : 0;
    ...
    if (self.response.contentStream !== null &&
      ...
      self._checkCompress()) {
      var acceptEncoding = self.request.getHeader('accept-encoding', "");
 ...
 **/
Context.prototype.statusTemplate = function (statusCode, options, headers) {
  var self = this;
  options = options || {};
 ...
 * 向浏览器响应一个状态
 **/
Context.prototype.status = function (statusCode, headers, end) {
  var self = this;
 ...
 * 向浏览器发送头信息
 ***/
Context.prototype.header = function (name, value) {
  var self = this;
  if (utils.isNull(name) || utils.isNull(value)) return;
...
Context.prototype.headers = function (headers) {
  var self = this;
  if (!headers) return;
  utils.each(headers, self.header.bind(self));
};
