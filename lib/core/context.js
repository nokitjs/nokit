 ...
 * 匹配合适的静态目录配置
 **/
Context.prototype._matchPublicConfig = function (withoutQueryStringUrl) {
    var self = this;
    ...
    _publicPath = self.server.resolvePath(pathParts[0]);
    return {
        path: _publicPath,
        remove: pathParts[1]
    }
...
Context.prototype._handleRequestPath = function () {
    ...
    req.setUrl = function (url, doNotUpdatePhysicalPath) {
        ...
        var publicConfig = self._matchPublicConfig(req.withoutQueryStringUrl);
        req.publicPath = publicConfig.path;
        if (!doNotUpdatePhysicalPath) {
            var filePath = req.withoutQueryStringURL;
            if (publicConfig.remove) {
                filePath = req.withoutQueryStringURL.replace(publicConfig.remove, "");
            }
            req.setPhysicalPath(req.publicPath + '/' + filePath);
        }
    };
