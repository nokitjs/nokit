...
Context.prototype._logBegin = function () {
    ...
    //开始记录日志
    self.logger.log([req.clientInfo.ip, req.method, req.url].join(';'));
};
...
Context.prototype._logEnd = function () {
    ...
    //结束记录日志
    self.logger.log([res.statusCode, self.useTime + 'ms'].join(';'));
...
Context.prototype._handleSession = function () {
    ...
    var req = self.request,
        res = self.response;
    self.sessionId = req.cookie.get(SESSION_ID_NAME);
    if (!self.sessionId) {
        self.sessionId = utils.newGuid().split('-').join('');
        res.cookie.add(SESSION_ID_NAME, self.sessionId, {
            "httpOnly": true,
            "secure": self.server.isHttps
        });
    }
    //--
    self.session = new self.server.SessionProvier(self);
