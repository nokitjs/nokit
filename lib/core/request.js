...
function Request(context) {
    var self = this;
    /**
     * 不用调用 http.IncomingMessage.call
     * 在 context 通过 Object.setPrototypeOf 改变 req.prototype.__proto__ 为 Request 的一个实例
     **/
 ...
 * 匹配合适的静态目录配置
 **/
Request.prototype._matchPublicConfig = function (withoutQueryStringUrl) {
    var self = this;
    var _publicPath = utils.each(self.context.configs.folders.public, function (exprText, _path) {
        if (exprText === "*") return;
    ...
    if (utils.isNull(_publicPath)) {
        _publicPath = self.context.configs.folders.public["*"];
    }
    var pathParts = _publicPath.split(' ');
    _publicPath = self.context.server.resolvePath(pathParts[0]);
    return {
        path: _publicPath,
        remove: pathParts[1]
    }
...
Request.prototype.setUrl = function (url, doNotUpdatePhysicalPath) {
    ...
    try {
        ...
        self.url = url;
        self.context.logger.warn(ex.message);
    }
    ...
    var publicConfig = self._matchPublicConfig(self.withoutQueryStringUrl);
    self.publicPath = publicConfig.path;
    if (!doNotUpdatePhysicalPath) {
        var filePath = self.withoutQueryStringURL;
        if (publicConfig.remove) {
            filePath = self.withoutQueryStringURL.replace(publicConfig.remove, "");
        }
        self.setPhysicalPath(self.publicPath + '/' + filePath);
    }
};
 ...
 * 初始化原始 request
 **/
Request.init = function (context, originRequest) {
    Object.setPrototypeOf(originRequest, new Request(context));
    originRequest.setUrl(originRequest.url);
