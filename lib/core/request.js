 ...
 * 匹配合适的静态目录配置
 **/
Request.prototype._matchPhysicalInfo = function(withoutQueryStringUrl) {
  var self = this;
  ...
  return {
    path: self.context.server.resolvePath(_publicPath),
    file: RegExp.$1 || withoutQueryStringUrl
  }
};
...
Request.prototype.setUrl = function(url, doNotUpdatePhysicalPath) {
  ...
  self.publicPath = physicalInfo.path;
  self.setPhysicalPath(physicalInfo.path + '/' + physicalInfo.file);
};
