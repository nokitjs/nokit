 ...
 * 处理 page 路径
 **/
Pagine.prototype._handlePath = function (pagePath) {
...
//编译一个模板
Pagine.prototype.compile = function (pagePath, options) {
    var self = this;
    pagePath = self._handlePath(pagePath);
    //检查缓存
    ...
    //编译
    options = options || {};
    options.extend = self._createExtendObject(pagePath);
    var buffer = fs.readFileSync(pagePath) || "";
    var page = tp.compile(buffer.toString(), options);
    self.cache[pagePath] = page;
    return page;
};

//创建模板扩展对象
Pagine.prototype._createExtendObject = function (pagePath) {
    var self = this;
    var extendObject = {
