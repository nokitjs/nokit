...
var generatorHelper = require("./generator-helper");

//------------------------------------------------------------------

var SYSTEM_CONFIG_NAME = 'system';
 ...
 * 处理配置文件 json > js
 **/
Server.prototype._resolveConfigFile = function(fromPath, configName) {
  var configFile = path.resolve(fromPath, "./" + configName);
...
Server.prototype._initConfigs = function(callback) {
  var self = this;
  //公共配置
  var systemConfigFile = self._resolveConfigFile(self.installPath, SYSTEM_CONFIG_NAME);
  var systemConfigs = utils.readJSONSync(systemConfigFile, true) || {};
  //应用配置 
  var configName = self.options.config || APP_CONFIG_NAME;
  var appConfigFile = self._resolveConfigFile(self.options.root, configName);
  var appConfigs = utils.readJSONSync(appConfigFile, true) || {};
  //环境配置
  var envName = self.options.env || process.env.NODE_ENV || "";
  if (!utils.isNull(envName)) {
    var appEnvConfigFile = self._resolveConfigFile(self.options.root, configName + '.' + envName);
...
Server.prototype._initSessionStore = function(callback) {
  ...
  generatorHelper.wrap(self.sessionStore);
...
Server.prototype._initGlobal = function() {
  ...
  if (fs.existsSync(gloablPath)) {
    ...
    generatorHelper.wrap(self.global);
  }
};
...
generatorHelper.wrap(Server.prototype);
