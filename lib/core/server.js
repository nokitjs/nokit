...
var APP_ENV_CONFIG_FILE_FORMAT = "./app.{env}.json";
...
Server.prototype._initConfigs = function (callback) {
    ...
    //应用配置 
    var appConfigFile = path.resolve(self.options.root, APP_DEFAULT_CONFIG_FILE);
    var appConfigs = utils.readJSONSync(appConfigFile) || {};
    //环境配置
    var envName = self.options.env || process.env.NODE_ENV || "";
    var appEnvConfigFile = path.resolve(self.options.root, APP_ENV_CONFIG_FILE_FORMAT.replace("{env}", envName));
    var appEnvConfigs = utils.readJSONSync(appEnvConfigFile) || {};
    //合并所有配置
    self.configs = {};
