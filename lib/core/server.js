...
Server.prototype._loadViewEngine = function () {
    var self = this;
    var viewEnginePath = self.resolvePath(self.configs.viewEngine);
 ...
 * 加载本地化资源
 **/
Server.prototype._loadLocale = function () {
    var self = this;
    self.locale = new Locale(self);
    self.locale.load();
};
...
Server.prototype._createListener = function () {
    ...
    return (function (req, res) {
        ...
        //执行 filter 事件
        context.filterInvoker.invoke('onRequestBegin', context, function (err) {
            if (err) {
