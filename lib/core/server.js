...
var APP_DEFAULT_CONFIG_FILE = path.normalize('./app.json');
var APP_CUSTOM_CONFIG_FILE_FORMAT = "./app.{env}.json";
var DEFAULT_HANDLER_NAME = "static";
var MODULE_DEFAULT_EXTENSION = '.js';
var UNCAUGHT_EXCEPTION_EXIT_DELAY = 3000;
...
Server.prototype._init = function (options) {
    ...
    self._handleUncaughtException();
    self._initConfigs();
    self._initLogger();
    self._initSessionProvider();
    self._initGlobal();
    self._loadParsers();
    self._loadFilters();
    self._loadHandlers();
 ...
 * 处理未知错误
 **/
Server.prototype._handleUncaughtException = function () {
    var self = this;
    process.on("uncaughtException", function (err) {
        self.logger.error(err);
        //发生未处理的异常时，延时后直接结束进程
        //否则 nodejs 进程将可能出现内存无法释放的问题
        setTimeout(function () {
            process.exit();
...
Server.prototype._initConfigs = function (callback) {
    ...
    //应用配置 
    var appConfigFile = APP_DEFAULT_CONFIG_FILE;
    var env = self.options.env || process.env.NODE_ENV;
    if (!utils.isNull(env) && env.trim() !== "") {
        appConfigFile = APP_CUSTOM_CONFIG_FILE_FORMAT.replace("{env}", env);
    }
    var fullAppConfigFile = path.resolve(self.options.root, appConfigFile);
    if (!fs.existsSync(fullAppConfigFile)) {
        //throw new Error('应用配置文件 "' + appConfigFile + '" 没有找到');
        fullAppConfigFile = path.resolve(self.options.root, APP_DEFAULT_CONFIG_FILE);
    }
    var appConfigs = utils.readJSONSync(fullAppConfigFile) || {};
    //合并
    self.configs = {};
...
Server.prototype._initLogger = function (callback) {
    ...
    self.LogWriter = self.require(logConfigs.provider);
    if (self.LogWriter.init) {
        self.LogWriter.init(self, callback);
        //全局日志对象
        self.logger = new Logger(new self.LogWriter(self));
    } else {
        throw new Error('日志提供程序没有发现 "静态 init 方法"');
    }
};
 ...
 * 初始化 session 提供程序
 **/
Server.prototype._initSessionProvider = function (callback) {
    var self = this;
    var sessionConfigs = self.configs.session || {};
    if (!sessionConfigs.enabled) {
        return;
    }
    self.SessionProvider = self.require(sessionConfigs.provider);
    if (self.SessionProvider.init) {
        self.SessionProvider.init(self, callback);
    } else {
        throw new Error('Session 提供程序没有发现 "静态 init 方法"');
    }
};
