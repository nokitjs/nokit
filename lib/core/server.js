...
Server.prototype._processExitHandler = function (code) {
    ...
    if (self.lastError) {
        ...
        self.logger.error(self.lastError, true);
    } else {
        self.logger.info(exitInfo, true);
    }
};
...
Server.prototype._isForbidden = function (context) {
    ...
    if (hosts && hosts.length > 0 && !utils.contains(hosts, req.clientInfo.host)) {
        return {
            "state": true,
            "message": 'Refused to use " ' + req.clientInfo.host + '" visit'
        };
    }
    //检查是否是禁止访问的资源
...
Server.prototype._handleRequest = function (context) {
    ...
    context.filterInvoker.invoke('onRequest', context, function (err) {
        ...
        if (!context.parser) {
            context.error('Content parser is not found');
            return;
        }
        //解析内容
