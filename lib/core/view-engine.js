...
const HAS_EXTNAME_REGEXP = /\.[a-z0-9]+$/i;
const CACHE_MAX_NUM = 100;

/**
 * 视图引擎
 * ViewEngine
 **/
const ViewEngine = module.exports = function (server) {
  var self = this;
  self.server = server;
  self.configs = server.configs.viewEngine;
  self._extnameExpr = new RegExp(self.configs.extname + '$');
...
ViewEngine.prototype._handlePath = function (viewPath) {
  ...
  if (HAS_EXTNAME_REGEXP.test(viewPath)) {
    return viewPath;
  } else {
    return viewPath + self.configs.extname;
  }
};
...
ViewEngine.prototype.compileFile = function (viewPath, options) {
  ...
  //检查缓存
  var cacheItem = self._cache.get(viewPath);
  if (cacheItem) {
    return cacheItem;
  }
  options = options || {};
