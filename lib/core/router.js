 ...
 * 释放资源
 **/
Router.prototype.dispose = utils.defineDisposer();
    ...
    utils.each(self.table, function(i, route) {
        ...
        if (route.matchExp.test(url)) {
            ...
            utils.each(route.keys, function(j) {
                ...
                params[route.keys[j]] = routeVal;
            });
            //clone route 对象
            var cloneRoute = utils.clone(route);
            cloneRoute.url = url;
            cloneRoute.params = params;
            cloneRoute = self._parseDynamicAction(cloneRoute);
            routeArray.push(cloneRoute);
        }
    });
