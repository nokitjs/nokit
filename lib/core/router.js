 ...
 * 解析占位符 key 定义
 **/
Router.prototype._parseKeyDef = function(keyDefStr) {
  var keyDef = {};
  var keyDefParts = keyDefStr.replace('{', '').replace('}', '').split(':');
  keyDef.name = keyDefParts[0];
  keyDef.exprStr = "(" + (keyDefParts[1] || DEFALUT_KEY_EXPR_STR) + ")";
  keyDef.str = keyDefStr;
  return keyDef;
};
...
Router.prototype.addOne = function(route) {
  ...
  utils.each(keyDefs, function(i) {
    ...
    route.keys[i] = keyDef.name;
    //将 "key 占位符" 的表达式，替换为 "key 正则表达式"
    route.exprStr = route.exprStr.replace(keyDef.str, keyDef.exprStr);
  });
  //生成 url 匹配测试表达式
  route.expr = new RegExp(route.exprStr, 'gim');
  //处理所有 route 的 method 
...
Router.prototype._createRouteInstance = function(srcRoute, url, params) {
  var self = this;
  var routeInstance = utils.clone(srcRoute);
...
Router.prototype.get = function(url, handleActionFromUrl) {
  ...
  utils.each(self.table, function(i, route) {
    ...
    //生成 params
    var params = {};
    utils.each(route.keys, function(i, key) {
