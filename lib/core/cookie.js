...
Cookie.prototype.set = function (name, value, options) {
    ...
    if (self.options.type != Cookie.TYPE_RESPONSE) {
        console.error('Can not write a cookie, because it is not an object "response.cookie"');
        return;
    }
    options = options || {};
...
Cookie.prototype.remove = function (name) {
    var self = this;
    if (self.options.type != Cookie.TYPE_RESPONSE) {
        console.error('Can not remove a cookie, because it is not an object "response.cookie"');
        return;
    }
