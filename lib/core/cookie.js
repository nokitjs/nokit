...
Cookie.prototype.set = function (name, value, options) {
    ...
    if (self.options.type != Cookie.TYPE_RESPONSE) {
        throw new Error('Can not write a cookie, because it is not an object "response.cookie"');
    }
    options = options || {};
...
Cookie.prototype.remove = function (name) {
    var self = this;
    if (self.options.type != Cookie.TYPE_RESPONSE) {
        throw new Error('Can not remove a cookie, because it is not an object "response.cookie"');
    }
    self.set(name, '', {
        "Max-Age": 0
    });
