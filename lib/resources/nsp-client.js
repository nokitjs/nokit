...
(function(owner) {
    ...
    owner.stringify = owner.stringify || function(obj) {
        switch (typeof(obj)) {
            case 'string':
                return '"' + obj.replace(/(["\\])/g, '\\$1') + '"';
            case 'array':
                return '[' + obj.map(owner.stringify).join(',') + ']';
            case 'object':
                if (isArray(obj)) {
                    var strArr = [];
                    var len = obj.length;
                    for (var i = 0; i < len; i++) {
                        strArr.push(owner.stringify(obj[i]));
                    }
                    return '[' + strArr.join(',') + ']';
                    ...
                    var string = [];
                    for (var p in obj) {
                        string.push(owner.stringify(p) + ':' + owner.stringify(obj[p]));
                    }
                    return '{' + string.join(',') + '}';
                }
            case 'number':
    ...
    owner.parse = owner.parse || function(str) {
...
(function(nsp, document, base64) {
    ...
    //查找表单
    var findForm = function(formId) {
        var theForm = document.getElementById(formId) || document.getElementsByTagName('form')[0];
        if (!theForm || theForm.length < 1) {
            theForm = document.createElement('form');
            theForm.style.display = 'display:none;';
    ...
    var isTopNSPElement = function(element) {
        while (element.parentNode || element.parentElement) {
            ...
            if (parent.getAttribute(SERVER_ID_NAME) != null) {
                return false;
            } else {
                element = parent;
            }
    ...
    var findNSPElements = function() {
        ...
        if (document.querySelectorAll) {
            nspElements = document.querySelectorAll('[' + SERVER_ID_NAME + ']');
        } else {
            var allElements = document.getElementsByTagName('*');
            each(allElements, function(i, element) {
                if (element.getAttribute(SERVER_ID_NAME) != null) {
    ...
    var getState = function() {
        ...
        each(nspElements, function(i, item) {
            var serverId = item.getAttribute(SERVER_ID_NAME);
            state[serverId] = item.outerHTML;
        });
        var stateJson = JSON.stringify(state);
        return base64.encode(stateJson);
    };

    //回调服务端方法
    nsp.call = function(methodName, formId) {
        var theForm = findForm(formId);
